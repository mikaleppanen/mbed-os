...
#ifdef TARGET_K64F
#define MULTICAST_FILTERING_SUPPORTED
#endif
...
void test_emac_multicast_filter_cb(int opt)
{
    ...
    if (next_step) {
        ...
        switch (test_step) {
            ...
            case 1:
                printf("STEP 1: set ipv6 multicast filter, test that input message is filtered\r\n\r\n");
                {
            ...
            case 3:
                printf("STEP 3: set ipv4 multicast filter, test that input message is filtered\r\n\r\n");
                {
    ...
    if (opt == INPUT) {
#ifdef MULTICAST_FILTERING_SUPPORTED
        if (receive == false) {
